...
_ARCHIVE_PACKAGE_SQL = """
    SELECT di.sop_instance_uid, di.series_instance_uid, di.modality,
           di.study_instance_uid, di.patient_id, di.file_path
...
_db_cfg = _config["postgres"]
db = PostgresInterface(
    host=_db_cfg["host"],
    database=_db_cfg["db"],
    user=_db_cfg["username"],
...
async def get_archive_package():
    try:
        ...
        for row in rows:
            ...
            db.execute_query(
                """
                INSERT INTO pacs_archive (sop_instance_uid, series_instance_uid, modality, study_instance_uid, patient_id, status)
                VALUES (%s, %s, %s, %s, %s, 'in_progress')
                ON CONFLICT (sop_instance_uid) DO UPDATE SET status = 'in_progress'
                """,
                (sop_uid, row[1], row[2], row[3], row[4]),
            )
            packages.append(
                {
